<template>
  <div>
    <h2 id="page-heading" data-cy="LocationHeading">
      <span v-text="$t('myApp.location.home.title')" id="location-heading">Locations</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('myApp.location.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'LocationCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('myApp.location.home.createLabel')"> Create a new Location </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && locations && locations.length === 0">
      <span v-text="$t('myApp.location.home.notFound')">No locations found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="locations">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('myApp.location.streetAddress')">Street Address</span></th>
            <th scope="row"><span v-text="$t('myApp.location.postalCode')">Postal Code</span></th>
            <th scope="row"><span v-text="$t('myApp.location.city')">City</span></th>
            <th scope="row"><span v-text="$t('myApp.location.stateProvince')">State Province</span></th>
            <th scope="row"><span v-text="$t('myApp.location.country')">Country</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="myApp.location.delete.question" data-cy="locationDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-location-heading" v-text="$t('myApp.location.delete.question', { id: removeId })">
